...
const NURSE_SURNAMES = ['mansell', 'amison', 'masterson', 'moreton', 'griffiths'];
...
// Color palette for nurses (cycle through colors)
const NURSE_COLORS = [
  '#3b82f6', '#14b8a6', '#8b5cf6', '#f59e0b', '#10b981', 
  '#ec4899', '#06b6d4', '#f97316', '#6366f1', '#a855f7'
];

// Slot type color mapping - matching the doctors calendar colors
const SLOT_TYPE_COLORS = {
  ...
  'baby immunisation': '#FEFB92',                  // Baby Imms variant
  ...
  'lunch': '#B3B3B3',                              // Lunch Break variant
  ...
  'sample testing': '#4282D4',                     // Sample testing - using blue
...
// Check if a name is a nurse
const isNurse = (fullName) => {
  if (!fullName) return false;
  const name = fullName.toLowerCase();
  return NURSE_SURNAMES.some(surname => name.includes(surname));
};

export default function NursesPerDayView() {
  ...
  useEffect(() => {
    const fetchSlots = async () => {
      ...
      try {
        ...
        console.log('Fetching nurse slots for date:', dateKey);
        ...
        // Filter for nurse slots only
        const nurseSlots = (calendarData || []).filter(slot => {
          const holderName = slot['Full Name of the Session Holder of the Session'];
          return isNurse(holderName);
        });
        ...
        if (appsErr) {
          ...
          // Filter for nurse appointments
          const nurseApps = (appsData || []).filter(app => {
            const holderName = app['Session Holders Full Name'];
            return isNurse(holderName);
          });
          
          setAppointments(nurseApps);
          
          // Extract unique EMIS numbers from booked appointments
          const emisNumbers = new Set();
          nurseApps.forEach(app => {
  ...
  // Process and organize slots by nurse, enriched with appointment data
  const nurseData = useMemo(() => {
    ...
    // Also create a lookup by time + nurse name (fallback)
    const appointmentByTimeMap = new Map();
    appointments.forEach(app => {
      const time = app['Appointment Time'];
      const nurse = app['Session Holders Full Name'];
      if (time && nurse) {
        const key = `${time}|${nurse}`;
    ...
    // Group by nurse and enrich with appointment data
    const byNurseMap = new Map();
    slots.forEach(slot => {
      const name = slot['Full Name of the Session Holder of the Session'];
      if (!byNurseMap.has(name)) {
        byNurseMap.set(name, []);
      }
      ...
      // Fallback: try matching by time + nurse name
      if (!appointment) {
      ...
      byNurseMap.get(name).push({
        ...slot,
        appointment: appointment || null
      });
    });

    // Sort each nurse's slots by time
    const result = [];
    byNurseMap.forEach((slotsForNurse, nurseName) => {
      const sorted = slotsForNurse.sort((a, b) => {
        const timeA = a['Appointment Time'] || '';
        const timeB = b['Appointment Time'] || '';
        return timeA.localeCompare(timeB);
      });
      result.push({ name: nurseName, slots: sorted });
    });
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {/* Grid */}
      {!loading && !error && nurseData.length === 0 && (
        <div style={{ 
          textAlign: 'center', 
          padding: 24, 
          color: '#6b7280',
          ...
          No nurse slots found for this date
        </div>
      )}
      ...
      {!loading && !error && nurseData.length > 0 && (
        <div style={{ 
          ...
          {/* Column-based layout - each nurse gets a column */}
          <div style={{
            display: 'grid',
            gridTemplateColumns: `repeat(${nurseData.length}, 1fr)`,
            gap: 16,
            minWidth: '100%'
          }}>
            {nurseData.map((nurseInfo, idx) => {
              const surname = extractSurname(nurseInfo.name);
              const headerColor = NURSE_COLORS[idx % NURSE_COLORS.length];
              
              return (
                <div key={nurseInfo.name} style={{
                  display: 'flex',
                  ...
                  <div style={{
                    ...
                    {nurseInfo.slots.length === 0 ? (
                      ...
                      nurseInfo.slots.map((slot, slotIdx) => {
                        const time = slot['Appointment Time'] || '';
                        ...
                        const violations = checkSlotCompliance({
                          type: slotType,
                          clinician: nurseInfo.name,
                          slotDuration: duration,
                          'Full Name of the Session Holder of the Session': nurseInfo.name,
                          'Slot Type': slotType,
